...
tripdata01 <- read.csv("202102-divvy-tripdata.csv")
...
new_tripdata02 <- clean_tripdatasub

new_tripdata02 <- new_tripdata01 %>%  filter(ridetime > 0) %>% unique() %>% select(ride_id, started_at, ended_at, ridetime,rideable_type)

write_csv(new_tripdata02, file="tripdata-2021-2.csv")
View(new_tripdata02)
