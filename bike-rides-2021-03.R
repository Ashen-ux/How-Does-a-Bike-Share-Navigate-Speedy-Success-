...
tripdata01 <- read.csv("202103-divvy-tripdata.csv")
...
new_tripdata03 <- clean_tripdatasub

new_tripdata03 <- new_tripdata01 %>%  filter(ridetime > 0) %>% unique() %>% select(ride_id, started_at, ended_at, ridetime,rideable_type)

write_csv(new_tripdata03, file="tripdata-2021-3.csv")
View(new_tripdata03)
